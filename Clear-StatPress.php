...
function CSP_sql_del() {
	global $wpdb;
	$query1 = "SELECT* FROM " . $wpdb->prefix . 'statpress '." WHERE
...
";
	...
	<tbody>";
	$temptemp = 0;//用来使表格两行背景颜色不同
	foreach ($output as $o) {
		if($temptemp == 0) { echo "<tr>"; $temptemp = 1;}
		else { echo "<tr class=\"alt\">"; $temptemp = 0;}
		echo "<td>" . $o->id . "</td>";
		echo "<td>" . $o->date . "</td>";
		echo "<td>" . $o->time . "</td>";
		echo "<td>" . $o->ip . "</td>";
		echo "<td>" . $o->urlrequested . "</td>";
		echo "<td>" . $o->statuscode . "</td>";
		echo "<td>" . $o->agent . "</td>";
		echo "<td>" . $o->referrer . "</td>";
		echo "<td>" . $o->search . "</td>";
		echo "<td>" . $o->nation . "</td>";
		echo "</tr>";
	}
...
?>
<style text="text/css">
.datagrid table { border-collapse: collapse; text-align: left; width: 100%; } .datagrid {font: normal 12px/150% Arial, Helvetica, sans-serif; background: #fff; overflow: hidden; border: 1px solid #006699; -webkit-border-radius: 3px; -moz-border-radius: 3px; border-radius: 3px; }.datagrid table td, .datagrid table th { padding: 0px 5px; }.datagrid table thead th {background:-webkit-gradient( linear, left top, left bottom, color-stop(0.05, #006699), color-stop(1, #00557F) );background:-moz-linear-gradient( center top, #006699 5%, #00557F 100% );filter:progid:DXImageTransform.Microsoft.gradient(startColorstr='#006699', endColorstr='#00557F');background-color:#006699; color:#FFFFFF; font-size: 15px; font-weight: bold; border-left: 1px solid #0070A8; } .datagrid table thead th:first-child { border: none; }.datagrid table tbody td { color: #00557F; border-left: 2px solid #E1EEF4;font-size: 12px;font-weight: normal; }.datagrid table tbody .alt td { background: #E1EEF4; color: #00557F; }.datagrid table tbody td:first-child { border-left: none; }.datagrid table tbody tr:last-child td { border-bottom: none; }
</style>
<?php
	$wpdb->query($query2);
	echo "<br>删除成功";
